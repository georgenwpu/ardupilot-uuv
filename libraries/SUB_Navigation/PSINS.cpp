...
//***************************  class CSysClbt  *********************************/
CSysClbt::CSysClbt(const CVect3 &pos00, double g00, int ka2pn):CSINSTDKF(39, 3)
{
	iter = 0;
	wibStatic = 0.1*DPS;
...
void CAutoDrive::ZUPTtest(void)
{
	if(sins.mmwb.flag==1 && odVel<1.0e-6) {
		...
		if(dwb<0.1*DPS && (double)sins.mmwb.maxRes<1.0*DPS && dfb<50*MG && (double)sins.mmfb.maxRes<50*MG) {
...
{
	CSINSGNSSOD::Update(pwm, pvm, dS, nn, ts, nSteps);
	if(odmeasOK) {
...
void CCNS::SetdT(double dUT10, double dTAI0)
{
	this->dUT1 = dUT10,  this->dTAI = dTAI0;  // dUT1=UT1-UTC, dTAI=TAI-UTC
	dTT = dTAI+32.184;
}
...
CMat3 CCNS::Precmat(double TT0)
{
	double T2 = TT0*TT0;  // TT ~= TDB
    double zeta  = 2306.2181*TT0+0.301880*T2;
    double theta = 2004.3109*TT0-0.426650*T2;
    double z     = 2306.2181*TT0+1.094687*T2;
    return CP = Rot(z*glv.sec,'z') * Rot(-theta*glv.sec,'y') * Rot(zeta*glv.sec,'z');
}

void CCNS::Equinox(double TT0)
{
	double T2 = TT0*TT0;  // TT ~= TDB
    eps  = (84381.448-46.8150*TT0)*glv.sec;
    double lp = (1287104.79305+129596581.0481*TT0-0.5532*T2)*glv.sec;
    double F  = (335779.526232+1739527262.8478*TT0-12.7512*T2)*glv.sec;
    double D  = (1072260.70369+1602961601.2090*TT0-6.3706*T2)*glv.sec;
    double Om = (450160.398036-6962890.5431*TT0+7.4722*T2 )*glv.sec;
...
//*********************  class CMahony AHRS  ************************/
CMahony::CMahony(double tau0, const CQuat &qnb0)
{
	SetTau(tau0);  this->tau = tau0;  tau1 = dtau = -1.0;
	qnb = qnb0;
	Cnb = q2mat(qnb);
...
void CMahony::SetTau(double tau0)	// https://zhuanlan.zhihu.com/p/582694093
{
	double beta = 2.146/tau0;
	Kp = 2.0*beta, Ki = beta*beta;
}

void CMahony::SetTau(double tau0, double tau10, double dtau0)   // time-varying tau = tau0 -> tau1
{
	SetTau(tau0);
	this->tau = tau0;  this->tau1 = tau10;  this->dtau = dtau0;
}
...
{
	double q0, q1, q2, q3, wx, wy, wz, fx, fy, fz, h11, h12, h21, h22; 
...
void CFileRdWt::Init(const char *fname0, int columns0)
{
	...
	else if(columns>0)			// txt file read
	{
		...
		{
			pos = ftell(rwf);
			(void)!fgets(line, sizeof(line), rwf);
			if(feof(rwf)) break;
...
int CFileRdWt::load(int lines, BOOL txtDelComma)
{
	if(columns<0)			// bin file read
	{
		...
		(void)!fread(buff, -columns, sizeof(double), rwf);
		if(lines==0) fseek(rwf, columns*sizeof(double), SEEK_CUR);
	}
	else					// txt file read
	{
		for(int i=0; i<lines; i++)  (void)!fgets(line, sizeof(line), rwf);
		if(txtDelComma)
		{
...
int CFileRdWt::loadf32(int lines)	// float32 bin file read
{
	...
	(void)!fread(buff32, -columns, sizeof(float), rwf);
...
CFileRdWt& CFileRdWt::operator>>(double &d)
{
	(void)!fread(&d, 1, sizeof(double), rwf);
	return *this;
}

CFileRdWt& CFileRdWt::operator>>(CVect3 &v)
{
	(void)!fread(&v, 1, sizeof(v), rwf);
	return *this;
}

CFileRdWt& CFileRdWt::operator>>(CQuat &q)
{
	(void)!fread(&q, 1, sizeof(q), rwf);
	return *this;
}

CFileRdWt& CFileRdWt::operator>>(CMat3 &m)
{
	(void)!fread(&m, 1, sizeof(m), rwf);
	return *this;
}

CFileRdWt& CFileRdWt::operator>>(CVect &v)
{
	(void)!fread(v.dd, v.clm*v.row, sizeof(double), rwf);
	return *this;
}

CFileRdWt& CFileRdWt::operator>>(CMat &m)
{
	(void)!fread(m.dd, m.clm*m.row, sizeof(double), rwf);
	return *this;
}
...
{
	if(f) { fprintf(f, "\t%f ", (double)ff); Flush(1); }
	return *this;
}
...
CFileLog& CFileLog::MArray(const char* varname, const double *var, int row, int clm, double scale, const char* comment)
{
	if(f) {
		...
		fprintf(f, "%s = [ %d %d ", varname, row, clm);
		if(comment) fprintf(f, "\t%% %s\n", comment); else fprintf(f, "\n");
		for(int j=0; j<row; j++) {
			for(int i=0; i<clm; i++) {
...
CAligni0fit::CAligni0fit(const CVect3 &pos00)
{
	Init(pos00);
}

void CAligni0fit::Init(const CVect3 &pos00)
{
	eth.Update(pos00);
	this->pos0 = pos00;
	tk = imu.tk = 0;
