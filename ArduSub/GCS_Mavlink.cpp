...
{
    return (uint32_t)ardusub.control_mode;
}

MAV_STATE GCS_MAVLINK_Sub::vehicle_system_status() const
{
    // set system as critical if any failsafe have triggered
    if (ardusub.any_failsafe_triggered())  {
...
void GCS_MAVLINK_Sub::send_banner()
{
    ...
    send_text(MAV_SEVERITY_INFO, "Frame: %s", ardusub.motors.get_frame_string());
}

void GCS_MAVLINK_Sub::send_nav_controller_output() const
{
    const Vector3f &targets = ardusub.attitude_control.get_att_target_euler_cd();
    mavlink_msg_nav_controller_output_send(
        chan,
        targets.x * 1.0e-2f,
        targets.y * 1.0e-2f,
        targets.z * 1.0e-2f,
        ardusub.wp_nav.get_wp_bearing_to_destination() * 1.0e-2f,
        MIN(ardusub.wp_nav.get_wp_distance_to_destination() * 1.0e-2f, UINT16_MAX),
        ardusub.pos_control.get_pos_error_z_cm() * 1.0e-2f,
        0,
        0);
}
...
{
    return (int16_t)(ardusub.motors.get_throttle() * 100);
}

float GCS_MAVLINK_Sub::vfr_hud_alt() const
{
    return ardusub.get_alt_msl();
}
...
void GCS_MAVLINK_Sub::send_scaled_pressure3()
{
    ...
    float temperature;
    if (!ardusub.temperature_sensor.get_temperature(temperature)) {
        return;
    }
...
bool GCS_MAVLINK_Sub::send_info()
{
    ...
    CHECK_PAYLOAD_SIZE(NAMED_VALUE_FLOAT);
    send_named_float("PilotGain", ardusub.gain);

    CHECK_PAYLOAD_SIZE(NAMED_VALUE_FLOAT);
    send_named_float("InputHold", ardusub.input_hold_engaged);

    CHECK_PAYLOAD_SIZE(NAMED_VALUE_FLOAT);
    send_named_float("RollPitch", ardusub.roll_pitch_flag);

    CHECK_PAYLOAD_SIZE(NAMED_VALUE_FLOAT);
    send_named_float("RFTarget", ardusub.mode_surftrak.get_rangefinder_target_cm() / 100.0f);
...
{
    const Parameters &g = ardusub.g;
    AP_AHRS &ahrs = AP::ahrs();
    AC_AttitudeControl_Sub &attitude_control = ardusub.attitude_control;
    ...
    if (g.gcs_pid_mask & 8) {
        const AP_PIDInfo &pid_info = ardusub.pos_control.get_accel_z_pid().get_pid_info();
...
{
    return ardusub.g.sysid_my_gcs;
}

bool GCS_Sub::vehicle_initialised() const {
    return ardusub.ap.initialised;
}
...
bool GCS_MAVLINK_Sub::try_send_message(enum ap_message id)
{
    ...
    case MSG_TERRAIN:
        ...
        ardusub.terrain.send_request(chan);
#endif
        break;
...
MAV_RESULT GCS_MAVLINK_Sub::_handle_command_preflight_calibration_baro(const mavlink_message_t &msg)
{
    ...
    if (!ardusub.control_check_barometer()) {
        return MAV_RESULT_FAILED;
    }
...
MAV_RESULT GCS_MAVLINK_Sub::_handle_command_preflight_calibration(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
    if (packet.y == 1) {
        ...
        //result = ardusub.mavlink_compassmot(chan);
...
MAV_RESULT GCS_MAVLINK_Sub::handle_command_do_set_roi(const Location &roi_loc)
{
    ...
    ardusub.mode_auto.set_auto_yaw_roi(roi_loc);
    return MAV_RESULT_ACCEPTED;
}
...
{
        if (!ardusub.set_mode(Mode::Number::POSHOLD, ModeReason::GCS_COMMAND)) {
            return MAV_RESULT_FAILED;
        }
        return MAV_RESULT_ACCEPTED;
}
...
{
        if (!ardusub.set_mode(Mode::Number::SURFACE, ModeReason::GCS_COMMAND)) {
            return MAV_RESULT_FAILED;
        }
        return MAV_RESULT_ACCEPTED;
}
...
MAV_RESULT GCS_MAVLINK_Sub::handle_MAV_CMD_CONDITION_YAW(const mavlink_command_int_t &packet)
{
        ...
        if ((packet.param1 >= 0.0f)   &&
            ...
            ardusub.mode_auto.set_auto_yaw_look_at_heading(packet.param1, packet.param2, (int8_t)packet.param3, (uint8_t)packet.param4);
            return MAV_RESULT_ACCEPTED;
        }
...
MAV_RESULT GCS_MAVLINK_Sub::handle_MAV_CMD_DO_CHANGE_SPEED(const mavlink_command_int_t &packet)
{
        ...
        if (packet.param2 > 0.0f) {
            ardusub.wp_nav.set_speed_xy(packet.param2 * 100.0f);
            return MAV_RESULT_ACCEPTED;
        }
        return MAV_RESULT_FAILED;
}
...
{
        if (ardusub.motors.armed() && ardusub.set_mode(Mode::Number::AUTO, ModeReason::GCS_COMMAND)) {
            return MAV_RESULT_ACCEPTED;
        }
        return MAV_RESULT_FAILED;
}
...
MAV_RESULT GCS_MAVLINK_Sub::handle_MAV_CMD_DO_MOTOR_TEST(const mavlink_command_int_t &packet)
{
        ...
        if (!ardusub.handle_do_motor_test(packet)) {
            return MAV_RESULT_FAILED;
        }
        return MAV_RESULT_ACCEPTED;
}
...
void GCS_MAVLINK_Sub::handle_message(const mavlink_message_t &msg)
{
    ...
    case MAVLINK_MSG_ID_MANUAL_CONTROL: {     // MAV ID: 69
        if (msg.sysid != ardusub.g.sysid_my_gcs) {
            break;    // Only accept control from our gcs
        }
        mavlink_manual_control_t packet;
        mavlink_msg_manual_control_decode(&msg, &packet);

        if (packet.target != ardusub.g.sysid_this_mav) {
            break; // only accept control aimed at us
        }

        ardusub.transform_manual_control_to_rc_override(
            packet.x,
            packet.y,
            packet.z,
            packet.r,
        ...
        ardusub.failsafe.last_pilot_input_ms = AP_HAL::millis();
    ...
    case MAVLINK_MSG_ID_SET_ATTITUDE_TARGET: { // MAV ID: 82
        ...
        if (packet.type_mask & (1<<6)) {
            ardusub.set_attitude_target_no_gps = {AP_HAL::millis(), packet};
        }
        ...
        if (is_equal(packet.thrust, 0.5f)) {
            ...
            climb_rate_cms = (packet.thrust - 0.5f) * 2.0f * ardusub.wp_nav.get_default_speed_up();
        } else {
            // descend at up to WPNAV_SPEED_DN
            climb_rate_cms = (packet.thrust - 0.5f) * 2.0f * ardusub.wp_nav.get_default_speed_down();
        }
        ardusub.mode_guided.guided_set_angle(Quaternion(packet.q[0],packet.q[1],packet.q[2],packet.q[3]), climb_rate_cms);
        break;
    }
    ...
    case MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED: {   // MAV ID: 84
        ...
        if ((ardusub.control_mode != Mode::Number::GUIDED) && !(ardusub.control_mode == Mode::Number::AUTO && ardusub.auto_mode == Auto_NavGuided)) {
            break;
        }
        ...
        if (!pos_ignore) {
            ...
            if (packet.coordinate_frame == MAV_FRAME_BODY_NED ||
                    packet.coordinate_frame == MAV_FRAME_BODY_FRD ||
                    packet.coordinate_frame == MAV_FRAME_BODY_OFFSET_NED) {
                ardusub.rotate_body_frame_to_NE(pos_vector.x, pos_vector.y);
            }
            ...
            if (packet.coordinate_frame == MAV_FRAME_LOCAL_OFFSET_NED ||
                    ...
                    packet.coordinate_frame == MAV_FRAME_BODY_FRD ||
                    packet.coordinate_frame == MAV_FRAME_BODY_OFFSET_NED) {
                pos_vector += ardusub.inertial_nav.get_position_neu_cm();
            }
        }
        ...
        if (!vel_ignore) {
            ...
            if (packet.coordinate_frame == MAV_FRAME_BODY_NED || packet.coordinate_frame == MAV_FRAME_BODY_FRD || packet.coordinate_frame == MAV_FRAME_BODY_OFFSET_NED) {
                ardusub.rotate_body_frame_to_NE(vel_vector.x, vel_vector.y);
            }
        }
        ...
        if (!pos_ignore && !vel_ignore && acc_ignore) {
            ardusub.mode_guided.guided_set_destination_posvel(pos_vector, vel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
        } else if (pos_ignore && !vel_ignore && acc_ignore) {
            ardusub.mode_guided.guided_set_velocity(vel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
        } else if (!pos_ignore && vel_ignore && acc_ignore) {
            ardusub.mode_guided.guided_set_destination(pos_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
        }
    ...
    case MAVLINK_MSG_ID_SET_POSITION_TARGET_GLOBAL_INT: {  // MAV ID: 86
        ...
        if ((ardusub.control_mode != Mode::Number::GUIDED)
            && !(ardusub.control_mode == Mode::Number::AUTO && ardusub.auto_mode == Auto_NavGuided)
            && !(ardusub.control_mode == Mode::Number::ALT_HOLD)) {
            break;
        }
        ...
        if (!z_ignore && ardusub.control_mode == Mode::Number::ALT_HOLD) { // Control only target depth when in ALT_HOLD
            ardusub.pos_control.set_pos_target_z_cm(packet.alt*100);
            break;
        }
        ...
        if (!pos_ignore && !vel_ignore && acc_ignore) {
            ardusub.mode_guided.guided_set_destination_posvel(pos_neu_cm, Vector3f(packet.vx * 100.0f, packet.vy * 100.0f, -packet.vz * 100.0f));
        } else if (pos_ignore && !vel_ignore && acc_ignore) {
            ardusub.mode_guided.guided_set_velocity(Vector3f(packet.vx * 100.0f, packet.vy * 100.0f, -packet.vz * 100.0f));
        } else if (!pos_ignore && vel_ignore && acc_ignore) {
            ardusub.mode_guided.guided_set_destination(pos_neu_cm);
        }
...
#if AP_TERRAIN_AVAILABLE
        ardusub.terrain.handle_data(chan, msg);
#endif
        break;
    ...
    case MAVLINK_MSG_ID_SYS_STATUS: {
        ...
        if ((packet.onboard_control_sensors_enabled & MAV_SENSOR_WATER) && !(packet.onboard_control_sensors_health & MAV_SENSOR_WATER)) {
            ardusub.leak_detector.set_detect();
        }
    }
        break;
...
#if AP_TERRAIN_AVAILABLE
            (ardusub.terrain.enabled() ? MAV_PROTOCOL_CAPABILITY_TERRAIN : 0) |
#endif
...
MAV_RESULT GCS_MAVLINK_Sub::handle_flight_termination(const mavlink_command_int_t &packet)
{
    if (packet.param1 > 0.5f) {
        ardusub.arming.disarm(AP_Arming::Method::TERMINATION);
        return MAV_RESULT_ACCEPTED;
    }
    return MAV_RESULT_FAILED;
}
...
{
    return static_cast<int32_t>(ardusub.get_alt_msl() * 1000.0f);
}

int32_t GCS_MAVLINK_Sub::global_position_int_relative_alt() const
{
    return static_cast<int32_t>(ardusub.get_alt_rel() * 1000.0f);
}
...
int16_t GCS_MAVLINK_Sub::high_latency_target_altitude() const
{
    ...
    //return units are m
    if (ardusub.control_mode == Mode::Number::AUTO || ardusub.control_mode == Mode::Number::GUIDED) {
        return 0.01 * (global_position_current.alt + ardusub.pos_control.get_pos_error_z_cm());
    }
    return 0;
...
uint8_t GCS_MAVLINK_Sub::high_latency_tgt_heading() const
{
    ...
    if (ardusub.control_mode == Mode::Number::AUTO || ardusub.control_mode == Mode::Number::GUIDED) {
        ...
        return wrap_360_cd(ardusub.wp_nav.get_wp_bearing_to_destination()) / 200;
    }
    return 0;      
}
...
uint16_t GCS_MAVLINK_Sub::high_latency_tgt_dist() const
{
    // return units are dm
    if (ardusub.control_mode == Mode::Number::AUTO || ardusub.control_mode == Mode::Number::GUIDED) {
        return MIN(ardusub.wp_nav.get_wp_distance_to_destination() * 0.001, UINT16_MAX);
    }
    return 0;
}
...
uint8_t GCS_MAVLINK_Sub::high_latency_tgt_airspeed() const
{
    ...
    if (ardusub.control_mode == Mode::Number::AUTO || ardusub.control_mode == Mode::Number::GUIDED) {
        return MIN((ardusub.pos_control.get_vel_desired_cms().length()/100) * 5, UINT8_MAX);
    }
    return 0;
}
