...
bool ModeAuto::init(bool ignore_checks) {
     if (!ardusub.position_ok() || ardusub.mission.num_commands() < 2) {
        return false;
    }

    ardusub.auto_mode = Auto_Loiter;

    // stop ROI from carrying over from previous runs of the mission
    // To-Do: reset the yaw as part of auto_wp_start when the previous command was not a wp command to remove the need for this special ROI check
    if (ardusub.auto_yaw_mode == AUTO_YAW_ROI) {
    ...
    ardusub.mission.start_or_resume();
    return true;
}
...
void ModeAuto::run()
{
    ardusub.mission.update();

    // call the correct auto controller
    switch (ardusub.auto_mode) {
...
void ModeAuto::auto_wp_start(const Location& dest_loc)
{
    ...
    if (!ardusub.wp_nav.set_wp_destination_loc(dest_loc)) {
        ...
        ardusub.failsafe_terrain_on_event();
        return;
    }
...
void ModeAuto::auto_wp_run()
{
    ...
    if (!motors.armed()) {
        ...
        ardusub.wp_nav.wp_and_spline_init();                                                // Reset xy target
        return;
    }
...
//  we assume the caller has set the circle's circle with ardusub.circle_nav.set_center()
//  we assume the caller has performed all required GPS_ok checks
void ModeAuto::auto_circle_movetoedge_start(const Location &circle_center, float radius_m, bool ccw_turn)
{
    // set circle center
    ardusub.circle_nav.set_center(circle_center);

    // set circle radius
    if (!is_zero(radius_m)) {
        ardusub.circle_nav.set_radius_cm(radius_m * 100.0f);
    }

     // set circle direction by using rate
    float current_rate = ardusub.circle_nav.get_rate();
    current_rate = ccw_turn ? -fabsf(current_rate) : fabsf(current_rate);
    ardusub.circle_nav.set_rate(current_rate);

    // check our distance from edge of circle
    Vector3f circle_edge_neu;
    ardusub.circle_nav.get_closest_point_on_circle(circle_edge_neu);
    float dist_to_edge = (inertial_nav.get_position_neu_cm() - circle_edge_neu).length();

    // if more than 3m then fly to edge
    if (dist_to_edge > 300.0f) {
        // set the state to move to the edge of the circle
        ardusub.auto_mode = Auto_CircleMoveToEdge;
        ...
        if (!ardusub.wp_nav.set_wp_destination_loc(circle_edge)) {
            // failure to set destination can only be because of missing terrain data
            ardusub.failsafe_terrain_on_event();
        }

        // if we are outside the circle, point at the edge, otherwise hold yaw
        float dist_to_center = get_horizontal_distance_cm(inertial_nav.get_position_xy_cm().topostype(), ardusub.circle_nav.get_center().xy());
        if (dist_to_center > ardusub.circle_nav.get_radius() && dist_to_center > 500) {
            set_auto_yaw_mode(get_default_auto_yaw_mode(false));
        } else {
...
{
    ardusub.auto_mode = Auto_Circle;

    // initialise circle controller
    ardusub.circle_nav.init(ardusub.circle_nav.get_center(), ardusub.circle_nav.center_is_terrain_alt(), ardusub.circle_nav.get_rate());
}
...
void ModeAuto::auto_circle_run()
{
    ...
    ardusub.failsafe_terrain_set_status(ardusub.circle_nav.update());

    float lateral_out, forward_out;
    ardusub.translate_circle_nav_rp(lateral_out, forward_out);
    ...
    attitude_control->input_euler_angle_roll_pitch_yaw(channel_roll->get_control_in(), channel_pitch->get_control_in(), ardusub.circle_nav.get_yaw(), true);
}
...
{
    ardusub.mode_guided.init(true);
    ardusub.auto_mode = Auto_NavGuided;
    // initialise guided start time and position as reference for limit checking
    ardusub.mode_auto.guided_limit_init_time_and_pos();
}
...
bool ModeAuto::auto_loiter_start()
{
    ...
    ardusub.wp_nav.set_wp_destination(stopping_point);
...
void ModeAuto::set_auto_yaw_look_at_heading(float angle_deg, float turn_rate_dps, int8_t direction, uint8_t relative_angle)
{
    ...
    if (relative_angle == 0) {
        // absolute angle
        ardusub.yaw_look_at_heading = wrap_360_cd(angle_deg * 100);
    } else {
        // relative angle
        if (direction < 0) {
            angle_deg = -angle_deg;
        }
        ardusub.yaw_look_at_heading = wrap_360_cd((angle_deg * 100 + curr_yaw_target));
    }

    // get turn speed
    if (is_zero(turn_rate_dps)) {
        // default to regular auto slew rate
        ardusub.yaw_look_at_heading_slew = AUTO_YAW_SLEW_RATE;
    } else {
        ardusub.yaw_look_at_heading_slew = MIN(turn_rate_dps, AUTO_YAW_SLEW_RATE);    // deg / sec
    }
...
void ModeAuto::set_auto_yaw_roi(const Location &roi_location)
{
    ...
    if (roi_location.alt == 0 && roi_location.lat == 0 && roi_location.lng == 0) {
        ...
        ardusub.camera_mount.clear_roi_target();
#endif  // HAL_MOUNT_ENABLED
    } else {
#if HAL_MOUNT_ENABLED
        // check if mount type requires us to rotate the sub
        if (!ardusub.camera_mount.has_pan_control()) {
            if (roi_location.get_vector_from_origin_NEU(ardusub.roi_WP)) {
                set_auto_yaw_mode(AUTO_YAW_ROI);
            }
        }
        // send the command to the camera mount
        ardusub.camera_mount.set_roi_target(roi_location);
...
bool ModeAuto::auto_terrain_recover_start()
{
    ...
    switch (ardusub.rangefinder.status_orient(ROTATION_PITCH_270)) {
    ...
    case RangeFinder::Status::Good:
        ardusub.auto_mode = Auto_TerrainRecover;
        break;
    ...
    ardusub.fs_terrain_recover_start_ms = AP_HAL::millis();

    // Stop mission
    ardusub.mission.stop();

    // Reset xy target
    ardusub.loiter_nav.clear_pilot_desired_acceleration();
    ardusub.loiter_nav.init_target();
...
void ModeAuto::auto_terrain_recover_run()
{
    ...
    if (!motors.armed()) {
        ...
        ardusub.loiter_nav.init_target();                                                   // Reset xy target
    ...
    case RangeFinder::Status::OutOfRangeLow:
        target_climb_rate = ardusub.wp_nav.get_default_speed_up();
        rangefinder_recovery_ms = 0;
        break;

    case RangeFinder::Status::OutOfRangeHigh:
        target_climb_rate = ardusub.wp_nav.get_default_speed_down();
        rangefinder_recovery_ms = 0;
        break;
    ...
    case RangeFinder::Status::Good: // exit on success (recovered rangefinder data)
        ...
        if (ardusub.rangefinder_state.alt_healthy) {
            ...
            if (AP_HAL::millis() > rangefinder_recovery_ms + 1500) {
                ...
                ardusub.failsafe_terrain_set_status(true); // Reset failsafe timers
                ardusub.failsafe.terrain = false; // Clear flag
                ardusub.auto_mode = Auto_Loiter; // Switch back to loiter for next iteration
                ardusub.mission.resume(); // Resume mission
    ...
    default:
        ...
        ardusub.failsafe_terrain_act();
        rangefinder_recovery_ms = 0;
        return;
    }

    // exit on failure (timeout)
    if (AP_HAL::millis() > ardusub.fs_terrain_recover_start_ms + FS_TERRAIN_RECOVER_TIMEOUT_MS) {
