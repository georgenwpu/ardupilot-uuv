...
bool ModeSurftrak::init(bool ignore_checks)
{
    ...
    if (!ardusub.rangefinder_alt_ok()) {
        ardusub.gcs().send_text(MAV_SEVERITY_INFO, "waiting for a rangefinder reading");
    } else if (ardusub.inertial_nav.get_position_z_up_cm() >= ardusub.g.surftrak_depth) {
        ardusub.gcs().send_text(MAV_SEVERITY_WARNING, "descend below %f meters to hold range", ardusub.g.surftrak_depth * 0.01f);
    }
...
bool ModeSurftrak::set_rangefinder_target_cm(float target_cm)
{
    ...
    if (ardusub.control_mode != Number::SURFTRAK) {
        ardusub.gcs().send_text(MAV_SEVERITY_WARNING, "wrong mode, rangefinder target not set");
    } else if (ardusub.inertial_nav.get_position_z_up_cm() >= ardusub.g.surftrak_depth) {
        ardusub.gcs().send_text(MAV_SEVERITY_WARNING, "descend below %f meters to set rangefinder target", ardusub.g.surftrak_depth * 0.01f);
    } else if (target_cm < (float)ardusub.rangefinder_state.min_cm) {
        ardusub.gcs().send_text(MAV_SEVERITY_WARNING, "rangefinder target below minimum, ignored");
    } else if (target_cm > (float)ardusub.rangefinder_state.max_cm) {
        ardusub.gcs().send_text(MAV_SEVERITY_WARNING, "rangefinder target above maximum, ignored");
    } else {
        success = true;
    }
    ...
    if (success) {
        ...
        ardusub.gcs().send_text(MAV_SEVERITY_INFO, "rangefinder target is %.2f meters", rangefinder_target_cm * 0.01f);

        // Initialize the terrain offset
        auto terrain_offset_cm = ardusub.inertial_nav.get_position_z_up_cm() - rangefinder_target_cm;
        ardusub.pos_control.set_pos_offset_z_cm(terrain_offset_cm);
        ardusub.pos_control.set_pos_offset_target_z_cm(terrain_offset_cm);
...
void ModeSurftrak::reset()
{
    ...
    ardusub.pos_control.set_pos_offset_z_cm(0);
    ardusub.pos_control.set_pos_offset_target_z_cm(0);
}
...
void ModeSurftrak::control_range() {
    float target_climb_rate_cm_s = ardusub.get_pilot_desired_climb_rate(channel_throttle->get_control_in());
    target_climb_rate_cm_s = constrain_float(target_climb_rate_cm_s, -ardusub.get_pilot_speed_dn(), g.pilot_speed_up);
...
void ModeSurftrak::update_surface_offset()
{
    if (ardusub.rangefinder_alt_ok()) {
        ...
        float rangefinder_terrain_offset_cm = ardusub.rangefinder_state.rangefinder_terrain_offset_cm;

        // Handle the first reading or a reset
        if (!HAS_VALID_TARGET && ardusub.rangefinder_state.inertial_alt_cm < ardusub.g.surftrak_depth) {
            set_rangefinder_target_cm(ardusub.rangefinder_state.inertial_alt_cm - rangefinder_terrain_offset_cm);
        }
        ...
        if (HAS_VALID_TARGET) {
            ...
            if (desired_z_cm >= ardusub.g.surftrak_depth) {
                // Adjust the terrain offset to stay below SURFTRAK_DEPTH, this should avoid "at_surface" events
                rangefinder_terrain_offset_cm += ardusub.g.surftrak_depth - desired_z_cm;
            }

            // Set the offset target, AC_PosControl will do the rest
            ardusub.pos_control.set_pos_offset_target_z_cm(rangefinder_terrain_offset_cm);
        }
    }
}
